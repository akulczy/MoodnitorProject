$(".notesBtn").click((event) =>{
    let userId = $(event.target).closest("tr").find(".uid").val();
    ...
    for(let entry of entries) {
        ...
        $("#entries-body").append(
            '<tr>' +
                ...
                '<td class="e-user">' + entry.SystemUser.name + ' ' + entry.SystemUser.surname + '</td>' +
                '<td class="e-date">' + entry.date + '</td>' +
                '<td class="e-title">' + entryTitle + '</td>' +
                `<td class="e-summary"><a class="linkNoStyle" href="/dashboard/specialist/users/summary/${entry.id}"><button class="btnGradBlueSm margin-auto">Overview</button></a></td>` +
...
$("#browse-title").click(() => {
    ...
    $.ajax({
        url: "/dashboard/specialist/entries/browse/title",
        method: "POST",
...
$("#browse-date").click(() => {
    ...
    $.ajax({
        url: "/dashboard/specialist/entries/browse/date",
        method: "POST",
        data: {date: dateValue},
        // Actions depending on the status code in the response
        statusCode: {
...
$("#browse-range").click(() => {
    ...
    $.ajax({
        url: "/dashboard/specialist/entries/browse/date/range",
        method: "POST",
